...
def main():
    ...
    """
    strategy_col = 'router_presence_threshold'
    hue_order = [0.0, 0.25, 0.5, 0.75]
    file_path = "08_router_presence_threshold.csv"
    ...
    if df is not None:
        ...
        fig.legend(handles, labels, loc='lower center', ncol=len(hue_order), bbox_to_anchor=(0.16, 0.01))
        ...
        for i, value in enumerate([5, 7, 9]):
            create_plot(axes[1, i], knn_sorensen_data, value, f'n_neighbors = {value}', 'KNN (sorensen)\nCorrect Percent',
                        hue_order, palette, strategy_col)
