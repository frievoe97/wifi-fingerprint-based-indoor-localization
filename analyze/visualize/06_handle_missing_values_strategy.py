...
def plot_bar(data, algorithm, ax, title, x_order, strategy_col):
    ...
    plot = sns.barplot(data=data,
                       ...
                       order=x_order,
                       palette=sns.color_palette("viridis", 3), errorbar=None)
    ax.set_title(title)
    ax.set_xlabel('')
...
def plot_line(data, algorithm, ax, title, x_order, strategy_col):
    ...
    sns.lineplot(data=data,
                 ...
                 hue_order=x_order, ax=ax, palette=sns.color_palette("viridis", 3), errorbar=None)
    ax.set_title(title)
...
def main():
    ...
    """
    strategy_col = 'handle_missing_values_strategy'
    hue_order = ["-100", "use_received", "zero"]
    file_path = "06_handle_missing_values_strategy.csv"
    ...
    if df is not None:
        ...
        fig, axes = plt.subplots(5, 3, figsize=(12, 8 ), sharey=True)
        ...
        for i, value in enumerate([5, 7, 9]):
            plot_line(knn_sorensen_data[knn_sorensen_data['algorithm_value'] == value],
                      'KNN (sørensen)', axes[1, i], f'n_neighbors = {value}', hue_order, strategy_col)
        ...
        fig.legend(handles, labels, loc='lower center', ncol=len(hue_order), bbox_to_anchor=(0.14, 0.01))
        ...
        for i, value in enumerate([5, 7, 9]):
            create_plot(axes[0, i], knn_euclidean_data, value, f'n_neighbors = {value}', 'KNN (euclidean)\nCorrect Percent',
                        hue_order, palette, strategy_col)

        for i, value in enumerate([5, 7, 9]):
            create_plot(axes[1, i], knn_sorensen_data, value, f'n_neighbors = {value}', 'KNN (sørensen)\nCorrect Percent',
                        hue_order, palette, strategy_col)
        ...
        for i, value in enumerate([0.5, 1, 5]):
            create_plot(axes[4, i], svm_rbf_data, value, f'C = {value}', 'SVM (RBF-Kernel)\nCorrect Percent', hue_order,
                        palette, strategy_col)
